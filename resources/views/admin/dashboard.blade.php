            ...
            <div class="bg-white  shadow-md p-6 rounded-md relative">
                ...
                <form action="{{ route('customers.store') }}" method="POST" class="space-y-4" id="create-customer-form">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-4">
                        ...
                        <div class="mb-4">
                            <label for="identifier" class="block text-sm font-medium text-gray-700">Customer
                                Identifier</label>
            ...
            <script>
                ...
                document.addEventListener("DOMContentLoaded",
                    function() {
                        const
                            openModalButton =
                            document.getElementById("open-create-customer-modal");
                        const
                            modal =
                            document.getElementById("create-customer-modal");
                        const
                            closeModalButton =
                            document.getElementById("close-create-customer-modal");
                        ...
                        @foreach ($products as $product)
                            <tr class="border-b border-gray-200 dark:border-gray-700">
                                    ...
                                    @if (strlen($product->description) > 100)
                                        <span id="shortDesc_{{ $product->id }}">
                                            ...
                                            <button onclick="toggleDescription('{{ $product->id }}')"
                                                id="readMoreBtn_shortDesc_{{ $product->id }}"
                                        ...
                                        <span id="fullDesc_{{ $product->id }}" style="display: none">
                                            {{ $product->description }}
                                            <button onclick="toggleDescription('{{ $product->id }}')"
                                                id="readMoreBtn_fullDesc_{{ $product->id }}"
                                                class="text-blue-500 hover:underline focus:outline-none">
                                                Read Less
                                            </button>
                                        </span>
                                ...
                                <td class="border px-4 py-2">
                                    <div class="text-center">
                                        @foreach ($product->categories as $category)
                                            <button
                                                class="bg-blue-200 hover:bg-blue-300 text-blue-800 font-semibold py-1 px-2 rounded-full mr-1 mb-1">
                                                {{ $category->name }}
                                            </button>
                                        @endforeach
                                    </div>
                                ...
                                <td class="border px-4 py-4 text-center">
                                    {{ \Carbon\Carbon::parse($product->created_at)->setTimezone('Asia/Kathmandu')->format('d/m/Y h:i A') }}
                                    {{ $product->createdBy->username ?? 'Unknown' }}</td>
                                <td class="border px-4 text-center py-4">
                                    {{ \Carbon\Carbon::parse($product->updated_at)->setTimezone('Asia/Kathmandu')->format('d/m/Y h:i A') }}
                                    {{ $product->updatedBy->username ?? 'Unknown' }}</td>
