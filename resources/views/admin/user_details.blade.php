        ...
        <div class="bg-white  shadow-md p-6 rounded-md relative">
            ...
            <form action="{{ route('Users.Store') }}" method="POST" class="space-y-4" id="create-user-form">
                ...
                <div class="text-center">
                    <button type="submit"
                        class="bg-white hover:bg-gray-100 text-gray-800 font-semibold py-2 px-4 border border-gray-400 rounded shadow">Create
                        User</button>
                </div>
            </form>
        </div>
                ...
                <tbody>
                    @foreach ($users as $user)
                        <tr class="border-b border-gray-200 dark:border-gray-700">
                            ...
                            <td class="border px-4 py-4">{{ $user->status == '1' ? 'Active' : 'Inactive' }}</td>
                            <td class="border px-4 py-4">{{ $user->type }}
                                [ {{ $user->createdBy->name }}]</td>
                            <td class="border px-4 py-4">
                                {{ \Carbon\Carbon::parse($user->created_at)->setTimezone('Asia/Kathmandu')->format('d/m/Y h:i A') }}
                                {{ $user->creator->username ?? '' }} </td>
                            <td class="border px-4 py-4">
                                {{ \Carbon\Carbon::parse($user->updated_at)->setTimezone('Asia/Kathmandu')->format('d/m/Y h:i A') }}
